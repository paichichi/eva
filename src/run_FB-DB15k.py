...
def main():
    ...
    elif "FB15K" in args.file_dir:
        filename = os.path.split(args.file_dir)[-1].upper()
        img_vec_path = "C:/Users/96446/Documents/GitHub/data/mmkg/" + filename + "/" + filename + "_id_img_feature_dict.pkl"
    else:
        split = args.file_dir.split("/")[-1]
        img_vec_path = args.file_dir + "/pkls/" + "zh_en_GA_id_img_feature_dict.pkl"
    ...
    # sf_vec_path = "data/DWY_data/dwy15k_" + split + "_sf_vec.npy"
    # sf_features = F.normalize(torch.Tensor(np.load(sf_vec_path)).to(device))
    # print("surface-form feature shape:", sf_features.shape)
    ...
    att_fc = nn.Linear(341, 100).to(device)
    img_fc = nn.Linear(4096, 200).to(device)
